...
우선순위는 "플레이어가 첫 5분에 만나는 순서"를 따른다.

| # | 작업 | 왜 지금 |
|---|---|---|
| 1 | **N4-3 5분 런 실플레이 튜닝** | 등급·페이싱 착지 후 실제로 5분 돌려보고 드랍률·XP곡선·난이도 조정. 숫자만 손봄 |
| 2 | **N6-1 첫 실행 흐름(FTUE)** | 첫 부팅 시 선택 생략하고 즉시 출정, 조작 오버레이 1회, 첫 개조 팝업에 설명 1줄 |
| 3 | **N3-13 무기·전리품 아이콘 아트 연결** | 지금 카드 아이콘이 전부 글자 플레이스홀더. codex 에셋 → 연결 |
| 4 | **N5-3 본거지(캠프) 최소판** | 런 사이 거점. 출정 버튼 + 엽전 표시 + 건물 자리만 |
| 5 | **N7-1 명부수형 메타 트리** | 엽전으로 영구 강화. 설화 `_02` 문법 |
| 6 | **N5-4 괴이록(도감)** | 처치한 몬스터·획득한 전리품 기록, 미발견은 ??? |
| 7 | **N8-1 두 번째 캐릭터** | 도사 완성 후 착수. 오너 시트 → 가공 방식 |
| 8 | **N8-2 두 번째 지역** | 공동묘지. 타일·몬스터·보스 1세트 |
| 9 | **N9-1 사운드** | BGM 2곡 + 타격/픽업/레벨업 효과음. 버스는 이미 있음 |
| 10 | **RZ-1 안드로이드 빌드 검증** | 실기기에서 터치·성능·해상도 확인 |
