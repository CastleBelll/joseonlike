...
| C1 | **N10-1 괴이 5종 — 루프를 깨는 놈들** | 그슨대(광원 안에서만 피격) · 야광귀(이속 아이템 절도, 체 오브젝트로 정지) · 삼두구미(무쇠·생달걀·버드나무 3약점 부위 파괴) · 영노(오브젝트·투사체를 먹어 100스택 = 플레이어 패배, DPS 체크) · 손님/호구별성(때리면 강해지는 보스, 헌납 3회 + 배웅 호위로만 퇴장). 전부 "때려 죽인다"를 한 번씩 깬다 — 이게 채택 이유다. **한 마리 = 한 피처로 분해할 것.** 실록 계열 아님(방언·무가·제주 구전), 원문 대조 불필요 |
