...
func puff(at: Vector2, radius: float, duration: float) -> void:
	global_position = at
	_radius = radius
...
func _draw() -> void:
	...
	draw_arc(Vector2.ZERO, ring_radius, 0.0, TAU, 24, UiPalette.DEATH_PUFF, RING_WIDTH)
	# A shrinking core disc keeps the puff readable at small radii.
	draw_circle(Vector2.ZERO, _radius * (1.0 - progress) * 0.5, UiPalette.DEATH_PUFF)
