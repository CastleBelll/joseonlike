...
## N9-54 명부수 사다리 곡선 — 부분 진척이 무엇을 사는가

`--meta=max`는 사다리의 끝만 말해준다. 중간이 걸을 만한지는 측정된 적이 없어
`--meta=gold:N`을 추가했다 — 예산을 주고 **매번 가장 싼 노드를 사는** 탐욕 구매로,
모아서 비싼 걸 한 방에 사는 게 아니라 실제로 사람이 오르는 방식이다.

런당 골드는 355~550 (3시드 측정, 평균 ~450). 도사 관련 트리 총액은 22,590골드
= 전부 열려면 약 50판. 다만 **이기는 데 전부는 필요 없다**:

| 예산 | ≈판수 | 산 랭크 | 결과 (seed 21/22/23) | 보스 잔여 HP |
|---|---|---|---|---|
| 0 | 0 | 0 | 패·패·패 | 2226 / 1001 / 2378 |
| 1,500 | ~3 | 11 | 패·패·패 | 1375 / 600 / 2390 |
| 3,000 | ~7 | 16 | 패·패·패(4:17 조기사) | 698 / 1142 / — |
| 4,500 | ~10 | 20 | **승**·패·패 | — / 705 / 2400 |

읽는 법: 보스 잔여 HP가 예산에 따라 실제로 내려간다(2226 → 1375 → 698). 진척이
숫자로 보인다는 뜻이고, 오너 방침("업그레이드를 해야 점점 쎄지지")대로 작동한다.
**첫 승리는 10판 근처**에서 나온다.

시드별 편차가 예산 효과보다 크다는 점은 유의 — seed 23은 4,500에서도 보스를 못
긁었다(잔여 2400). 이건 사다리가 아니라 **빌드 운**이다(재료 0개로 끝나는 판은
N9-24에 기록됨). 사다리 자체를 재조정할 근거는 이 측정에 없다.

봇 기준이라 사람은 더 잘한다. 이 표는 상한이 아니라 바닥으로 읽어야 한다.

## N9-68 몬스터 밀도 상향 (오너: "로그라이크 장르답게 몬스터가 더 많고")

측정해보니 필드는 **웨이브가 아니라 상한에 묶여 있었다** — 최대 동시 84 =
`live_cap` 84. 상한만 올리면 93→123에서 멈춘다(이번엔 공급이 병목). 그래서 둘
다 손봤다.

- `live_cap` 84 → **130**
- 도입부 이후 웨이브 물량 ×1.6 (+280마리). **도입부(20초 창)는 그대로 뒀다** —
  FTUE와 N6-2 오프닝 계약이 그 숫자에 걸려 있다.

| | 이전 | 이후 |
|---|---|---|
| 최대 동시 | 84 | **117 / 130 / 130** (3시드) |
| fps 평균 | 145 | 141 / 145 / 144 |
| fps 최저 | 144 | **132** (가장 붐빌 때 한 번) |
| 패배 시각 | 375s | 400 / 367 / 386s |

**상한을 160까지 올려봤다가 130으로 내렸다.** 160에서는 실제로 160을 채우고
fps가 133으로 떨어진다. 데스크톱에선 8% 남짓이지만 오너는 **모바일 웹**에서
플레이하고 그쪽이 훨씬 약하다 — 최악값을 낮게 묶는 편이 옳다. 물량(×1.6)은
그대로 두었으므로 런의 더 많은 구간이 빽빽해지고, 봉우리만 잘린다.
...
## N10-5 레벨업 카드 배합 (2026-08-26, owner: 3 직업 밸런스를 맞춰보자 —
## 구성이 너무 엉성한 것 같다)

세 직업을 같은 조건으로 재보니 문제는 능력치가 아니라 **카드 풀 산수**였다.
`LevelUp.pick`은 풀 전체를 균등 셔플하므로 무기 카드가 뽑힐 확률이 그 직업
카테고리에 레벨업 가능 무기가 몇 종 있느냐를 그대로 따라간다:

| 직업 | 레벨업 가능 무기 | 풀 내 무기 지분 |
|---|---|---|
| 도사 | spiritual 7종 | 약 38% |
| 무사 | melee 1종 (환도) | 약 7% |
| 궁수 | ranged 1종 (활) | 약 7% |

전후 (bamboo_forest, `--speed=4 --seed=7 --runs=2`, 2번째 런 기준 — 첫 런은
튜토리얼 길이라 읽지 않는다):

| | 도사 | 무사 | 궁수 |
|---|---|---|---|
| 전 | 승 363.8s · 1126킬 · 무기 3종 lv8 | 패 347.6s · 147킬 · 환도 **lv1** | 패 354.3s · 419킬 · 활 lv6 |
| 후 | 승 372.5s · 1146킬 · 무기 4종 lv8 | 패 399.9s · 243킬 · 환도 **lv8** | 패 374.9s · 479킬 · 활 **lv8** |

무사가 환도 lv1로 런을 끝내고 있었다는 뜻은 개조 게이트(`level_required` 5)에
영영 못 닿았다는 뜻이고, 그래서 참격검·귀검·화염검·쌍검은 데이터에만 있는
무기였다. 무기 슬롯 4칸 중 3칸도 항상 비어 있었다.

`weapon_card_share` 0.55는 도사의 기존 38%보다 높다 — 도사가 함께 세지지만
승리 시각이 363.8→372.5s로 오히려 조금 늦어졌다. 무기 카드가 늘어난 만큼
패시브가 줄어 총 전력은 비슷하고, 대신 **무엇을 키우는 런인지가 보인다**.

고치지 않고 남긴 것: 무사·궁수는 여전히 패배한다. 원인이 다르다 — 레벨업으로
얻을 수 있는 무기가 각 1종뿐이라 슬롯 3칸이 비고, 궁수는 actives가 아예 없어
도력 패시브가 100% 무효 카드다. 그건 로스터·정체성 문제이므로 별도 피처로
간다. 여기서 같이 만졌으면 무엇이 효과였는지 알 수 없게 된다.


## N10-6 죽은 카드 게이트 + 하네스 재현성 결함 (2026-08-26)

`_passive_has_a_customer`가 검사하던 것은 연쇄·봉인·불씨 셋뿐이었다. 실제로는
직업마다 **절대 쓸 수 없는 패시브**가 더 있었다:

| 직업 | 무효 카드 | 왜 |
|---|---|---|
| 무사 | 다중 투사, 신속 투사 | `melee_arc`는 Projectile을 안 날린다 |
| 궁수 | 도력 | actives가 0개 — 도력은 술법 피해만 올린다 |

이제 투사체 패시브는 투사체 mechanic 무기를 하나라도 들었을 때만, 도력은 피해가
있는 active를 가진 직업에게만 나온다. 이미 쌓은 패시브는 계속 성장한다(N9-110).

### 재현성 결함 — 이 프로젝트의 측정 전제가 틀렸다

전후 비교 중 같은 시드가 같은 결과를 안 낸다는 것을 발견했다. 도사 seed 7이 한
번은 승 374.2s, 다음 실행에서 패 374.9s. `--fixed-fps 60`으로도 안 잡힌다.

원인: 크리티컬 판정이 전역 `randf()`를 쓴다(`auto_weapon.gd:282`, `summon.gd:129`,
`ward.gd:148`). 그 전역 스트림을 연출·스프라이트 프레임·XP 구슬 위상
(`xp_orb.gd:47`, `stage_field.gd:454`, `chain_bolt.gd:58`, `curse_creep.gd:58`)이
함께 소비하므로, 프레임 타이밍이 조금만 달라져도 크리티컬이 어디서 터지는지가
갈린다. 필드 시드만 `seed(_run_seed)`로 고정돼 있었다.

**따라서 이 문서의 과거 전후 비교에는 승패 한 판 수준의 노이즈가 섞여 있다.**
단일 시드 1런으로 판정한 항목은 재확인 없이 신뢰하면 안 된다. 앞으로는 최소
5시드 집계로 읽는다. 전투 RNG를 스테이지 소유 스트림으로 분리하는 것은 별도
피처로 큐에 올린다.

### 5시드 집계 (대숲, `--seed=7 --runs=5 --speed=4`)
...
N10-6 이전의 5시드 기준선이 없어 이 표는 **절대 수치일 뿐 전후 비교가 아니다**.
도사 1/5는 낮은데, 도사는 이 게이트에 거의 걸리지 않으므로(투사체 무기와 피해
있는 active를 모두 보유) 원인은 다른 곳일 가능성이 높다 — 유력한 후보는 N10-5의
`weapon_card_share` 0.55가 도사의 패시브 지분을 62%에서 45%로 줄인 것이다.
로스터를 채우기 전에 share를 만지면 두 번 튜닝하게 되므로, 무사·궁수 무기를
넣은 뒤 share와 직업 스탯을 함께 잰다.


## N10-7 무사 뿌리 무기 2종 (2026-08-26)

무사는 레벨업으로 얻을 수 있는 무기가 환도 하나뿐이었다. 무기 슬롯은 4칸인데
3칸이 구조적으로 비었고, 개조 분기 셋(참격검·귀검·화염검)이 전부 그 한 뿌리에
매달려 있어 하나를 고르면 나머지 둘은 그 런에서 사라졌다.

뿌리를 셋으로 늘렸다. 세 무기가 **리치·속도·넉백** 축에서 갈린다:

| 무기 | 원호 | 리치 | 피해 | 쿨다운 | 관통 | 넉백 |
|---|---|---|---|---|---|---|
| 환도 | 120° | 90 | 16 | 0.80 | 2 | 1.6 |
| 월도 | 210° | 135 | 30 | 1.30 | 3 | 2.2 |
| 편곤 | 100° | 78 | 11 | 0.42 | 1 | 3.2 |

개조는 새 재료를 만들지 않고 이미 도는 것에 얹었다 — 월도+귀철 → 청룡언월도(240°,
리치 150), 편곤+숫돌 → 마상편곤(쿨다운 0.36, 넉백 3.6).

### 측정 (대숲, `--seed=7 --runs=5 --speed=4`)

| | 승/5 | 킬 범위 | 끝난 빌드 |
|---|---|---|---|
| 전 (뿌리 1종) | 0 | 163–792 | 환도 1종 |
| 후 (뿌리 3종) | **4** | 188–1140 | 무기 3종 + 개조 |

유일한 패배 런은 환도 하나만 뽑힌 런(188킬)이다. 나머지 넷은 3무기 빌드로 승리했고
킬 1021~1140 — 도사와 같은 대역이다. 슬롯이 차니까 직업이 성립한다.

아이콘은 32그리드 플레이스홀더를 직접 그렸다(`asset/ui/build_melee_icons.py`).
검증기가 아이콘 없는 무기를 막아서 로스터가 못 들어간다 — 오너 그림이 오면 교체한다.


## N10-12 직업 정체성 — 스탯 축 · 기본 스킬 · 술법 (2026-08-26,
## owner: 캐릭터 별 특화, 특징들을 제대로 잡은 스킬들, 기본 스킬 등 정리를 빡세게)
...
도사가 hp도 중간 속도도 중간이라 **조용히 가장 안전한 직업**이었다. "약하지만 광역"이
숫자 어디에도 없었다. 무적 시간은 셋이 완전히 같았다.
...
| 이동 속도 | **92** | **76** | **110** |
| 무적 시간 | **0.8** | **1.0** | **0.72** |
| 한 줄 | 판을 깔아 태운다 | 붙어서 밀어낸다 | 거리를 사서 꿰뚫는다 |

각 직업이 세 축 중 둘에서 극단이다. 무사는 느리고 두껍고 맞아도 오래 버틴다(무적 1.0초는
연속 피격을 실제로 막는다). 궁수는 얇고 빠르고 한 대가 치명적이다(0.72초는 두 번째 타격이
훨씬 빨리 들어온다는 뜻이다).

### 기본 스킬 — 새 개념, 직업당 하나

지금까지 직업은 시작 무기와 무기 카테고리로만 갈렸다. 그래서 첫 레벨업 카드를 뽑기 전까지
직업이 직업으로 읽히지 않았다.

| | 이름 | stat | 값 |
|---|---|---|---|
| 도사 | 음양(陰陽) | `area_scale` | +0.12 |
| 무사 | 철심(鐵心) | `defense` | +0.12 |
| 궁수 | 보사(步射) | `projectile_speed` | +0.20 |

**의도적으로 패시브 풀이 이미 쓰는 stat을 골랐다.** `_passive_bonus()` 한 곳에서 합산되므로
읽는 쪽(무기·술법·피해 계산) 누구도 새 개념을 배우지 않는다. 영구이고 중첩되지 않는다 —
그게 직업과 빌드를 가르는 선이다.
...
| | 회피·생존 | 판 뒤집기 |
|---|---|---|
| 도사 | 축지 (blink 140px, 무적 1.0초) | 벽사진 (burst 220r, 250) |
| 무사 | 철벽 (guard 4초, 받는 피해 0.3배) | 참격 (cleave 150r, 130°, 140) |
| 궁수 | **철질려** (trap, 130r, 감속 0.45, 6초, 틱 9) | **일제사** (cleave 520r, 20°, 150) |

**궁수에게 blink를 주지 않은 것이 이 설계의 핵심이다.** 도사가 이미 순간이동을 쓰므로 같은
탈출기를 주면 두 직업이 같게 읽힌다. 궁수는 **부정한 땅**으로 거리를 산다 — 마름쇠를 뿌리며
물러나는 것이 활의 생존기이고, 순간이동은 그 대가를 공짜로 없앤다. 철질려(鐵蒺藜)는 실재한
조선 무기다.

일제사는 새 메커닉이 아니라 **참격과 같은 `cleave` 수학**이고 숫자가 정반대다(20°×520 대
130°×150). 쓸어담기와 일직선 관통의 차이가 그 두 숫자에 다 들어 있다. 새 엔진 타입은
`trap` 하나뿐이며 결계가 쓰는 `Ward`를 그대로 재사용한다 — 그 메커닉이 이미 받은 수정을
전부 물려받는다.
...
|---|---|---|---|
| `bow` | 각궁 | straight | 균형 |
| `soenoe` | 쇠뇌 | pierce | 느리고 무겁고 줄을 뚫는다 (관통 4) |
| `pyeonjeon` | 편전 | straight | 짧고 빠른 애기살, 2발 |

개조는 새 재료 없이 기존 드롭으로 — 쇠뇌+귀철 → 수노기, 편전+숫돌 → 육량전.

### 검증기 계약

`trap`은 `damage`가 양수여야 하고 `ward{radius_px, duration_sec, tick_sec}`가 전부 양수여야
하며 `slow_scale`은 (0, 1) 열린 구간이다 — 1이면 아무것도 안 늦추고 0이면 영구 정지라
탈출기가 아니라 스턴이 된다.

### 테스트를 형태 검사로 바꿨다

`test_player_hp_reads_characters_json`과 `test_player_speed_reads_characters_json`이 도사의
hp 120과 속도 90을 리터럴로 들고 있어서 **직업 밸런스를 만질 때마다 깨졌다.** 그 테스트가
지키려는 계약은 "로더가 파일을 읽는가"이지 "120인가"가 아니므로, 이제 테스트도 파일을 읽어
대조한다.
...
| | 이전 | 이후 | 킬 범위 |
|---|---|---|---|
| 도사 | 1승/5 | **4승/5** | 282–1165 |
| 무사 | 4승/5 | **4승/5** | 214–1166 |
| 궁수 | 0승/5 | **3승/5** | 408–1152 |

**도사는 hp를 120에서 105로 깎았는데 승률이 올랐다.** 음양의 범위 +12%가 광역 직업에
그만큼 값을 한다 — 몸을 잃고 판을 얻은 거래가 성립했다.

**무사는 4승을 유지했다.** hp 140→170에 무적 0.8→1.0, 철심까지 붙었는데도 늘지 않은 것은
속도 80→76의 대가가 실제로 작동한다는 뜻이다. 방어를 얻고 기동을 잃는 거래가 상쇄됐다.

**궁수는 무적 시간을 한 번 조정했다.** 0.65에서는 2승/5였는데, 패배 런의 킬이 749·893·972로
높았다 — 못 싸워서 지는 게 아니라 한 대가 치명적이라 오래 못 버티는 것이었다. hp를 올리면
"얇다"가 사라지므로 축을 건드리지 않고 무적만 0.72로 완화했고 3승/5가 됐다. 그 뒤 패배
1런도 킬 1124로 끝까지 싸웠다.

고치지 않고 남긴 것: 도사의 뿌리 무기가 7종인데 무사·궁수는 3종이다. 카드 배합(N10-5)이
뽑힐 확률은 이미 균등하게 만들었지만 **고를 수 있는 폭**은 여전히 두 배 넘게 차이 난다.
지금 승률이 4/4/3으로 모여 있어 급하지 않고, 손댈 때는 도사를 줄이는 게 아니라 무사·궁수를
채우는 방향이다.


## 2026-08-29 — N11 난이도 상향 1차 (오너: "난이도가 좀 많이 낮은거같아")

체감의 번역: 정상 봇이 전 랭크에서 **hp 만땅으로 보스에 도달**하면 필드 압박이 0이다.

### 측정으로 드러난 두 결함 (대숲, seed=7, runs=2, speed=4, 2번째 런 기준)

1. **필드 무접촉.** 도사 이속 92 vs 잡몹 40~72 — 최속 추격자(cursed_hound 95)를
   빼면 걷기만으로 전부 회피된다. 순행·수난·역병 전 랭크에서 hp 105/105 보스 도달,
   회복 사용 0.
2. **사다리 비단조.** 역병(4) xp_mult 1.45가 위협 증가를 상쇄 — 수난(2)은 보스에서
   패배하는데 역병은 **승리**했다(빠른 성장으로 위협을 추월).

### 바꾼 것

- `monsters.json` 추격 속도 일괄 **x1.18** (29종; 장승 15·파생 정예 0 제외).
  중간층이 플레이어의 75~85%가 되어 포위가 성립한다.
- `difficulties.json` xp_mult 평탄화 1.1/1.2/1.3/1.45 → **1.05/1.1/1.15/1.2**
  (성장 보상은 골드·메타 몫), enemy_speed_mult 1.05/1.1/1.15/1.22 →
  **1.08/1.15/1.24/1.32**.
...
| 랭크 | 이전 | 이후 |
|---|---|---|
| 순행(0) | 승리, hp 105/105, 회복 0 | 승리, 회복 2 사용 (진입 랭크는 이겨야 한다) |
| 수난(2) | **패배** 367s | 승리 356s, 회복 4 사용 |
| 역병(4) | **승리** 373s | **패배** 380s |

사다리가 다시 단조가 됐고, 필드가 회복 경제를 실제로 소비시킨다. 순행이 여전히
무난한 것은 의도 — 근본 난이도 재설계(아이들러식 메타 성장과 함께)는 N11 로드맵의
별도 피처로 간다.

재보고도 안 고친 것: 잡몹 damage 자체(속도 축만 만졌다 — 두 축을 한 번에 올리면
어느 쪽이 효과였는지 알 수 없다). 다음 패스에서 필요하면 damage를 잰다.


## 2026-08-30 — N11-9 수련 대확장 증분 1 (오너: "수련에 굉장히 많은 트리, 캐릭터별로 다르게")

가지 확장: 석주 2→10노드(버티기·근접·포위 — 체력/방비/무골/불굴/진각/태산 걸음),
월영 2→10노드(회피·정밀·투사 — 월영 보법/형안/관통·갈래 시위/급소/그림자 걸음/가호/감),
우치 13→15노드(오행 조화 = NB 오행 축 선행 훅, 광대한 법식). 총 51→**69노드**.

- 비용: 가지 신규 합계 석주 13,670 / 월영 14,100 / 우치 7,700냥 + 재료 빌(지역
  전리품 — cast_iron/willow_branch/raw_egg가 처음으로 수련 화폐에 편입).
- stat_caps는 **"한 캐릭터가 트렁크+연마+자기 가지로 도달 가능한 최대치" 자동
  산출**로 전환(_caps_note) — 캡 낭비 0이 데이터 규칙이 됐다.
- 측정(대숲 seed 7, runs 2): 석주 신규 프로필 1승 1패(보스전 패배 — 캡 상향으로
  붕괴 없음), --meta=max 2승(만렙 메타가 이기는 것이 아이들러 약속의 이행).
  599/599 · 19 json · sweep PASS.
- 미착수(증분 2): 마일스톤 게이트(첫 승리·괴이록 페이지 수로 단계 해금), NB 무기
  격자와의 stat 동시 배선.

## 2026-08-30 — N11-6a 연마 5뿌리 사슬화 + 렌더 픽셀 스냅 (오너: "갈래가 다양하고 적당한 재화로", "뛰는 모션 픽셀 깨짐")

체감 번역: ① 연마 17노드가 전부 평면(requires 없음) — 마인드맵인데 갈래가 없다.
② 달리기 중 스프라이트 지글거림 — stage.gd CAMERA_BASE_ZOOM 1.15 × sqrt(면적비)로
줌이 항상 비정수, 서브픽셀 크롤. 스냅 설정이 project.godot에 없었다.

변경:
- 연마 17노드를 5뿌리 사슬로 재구성 — 공격(공격력→공속→치명率→치명피해),
  술법(술력→범위→소신→{연쇄, 부적속도} 분기), 생존(체력→방비),
  신속(이속→자석→{행운, 오성} 분기), 투사(탄속→탄수). 뿌리 첫 비용 70~100냥,
  깊이당 첫 비용 상승(선행-비용 검증 규칙 준수), 랭크 성장 ×2.0.
- project.godot [rendering]에 snap_2d_transforms_to_pixel + snap_2d_vertices_to_pixel.

측정 (귀환 프로필 수입 1,050냥/판 기준, greedy ding 시뮬, 우치 시점):
| | 사슬화 전 (평면) | ×2.2 성장 | **×2.0 성장 (채택)** |
|---|---|---|---|
| 총비용 | 107,790 | 138,670 | 117,620 |
| 만렙 판수 | 103 | 131 | 113 |
| 1판 딩 | 14 | 12 | 12 |
| 최대 가뭄 | 2판 | 8판 | **6판** |

가뭄 6판은 만렙 직전 꼬리(마지막 랭크 3,000냥+ 구간)에만 나타난다 — 아이들러
후반 정체로 허용. 검증: 600/600 · 19 json · layout_sweep PASS · meta_flow
bank/buy/verify PASS. 재보고도 안 고친 것: 효과 수치(per_rank)는 그대로 — 비용
축만 만졌다.


## 2026-08-30 — N11-4 개조 해금 1차 가격

전 레시피 일괄 엽전 200 + 자기 촉매 재료 2개 (22종 = 총 4,400냥 + 재료 44개).
근거: 귀환 수입 1,050냥/판이라 판당 4~5개 해금 가능 — 초반에 몇 개 고르고 중반에
채우는 속도. 경사(무기별/결과 등급별 차등)는 N11-6 경제 패스에서 창고 유입량을
잰 뒤 붙인다. 런 내 발동 조건(무기 Lv5 + 재료 픽업)은 불변.


## 2026-08-31 — N11-14 공용 탭 폭 우선 재배선

라디얼 화면이 담을 수 있는 팔 길이는 세로 540x960에서 3~4단이다(반지름 ~227px ÷
56px 원판). 공용 탭은 뿌리 2 x 깊이 9라 원판이 24px까지 축소돼 읽히지 않았다.
같은 17노드를 5뿌리 x 최대 3단으로 재배선했다 — 몸(철골→돌가죽→회생), 술(부적
연마→빠른 손→신속 부적), 걸음(바람 걸음→혼 끌기→긴 호흡), 눈(엽전 눈→혜안→
{행운, 범의 눈, 이갈이}), 준비(선수→첫 발견→네 번째 패).

비용은 한 냥도 바꾸지 않았고 선행-비용 규칙(자식 첫 비용 > 부모)은 17배선 전부
검사했다. 시뮬(수입 1,050냥/판, greedy): 만렙 113판 · 1판 12딩 · 최대 가뭄 5판
(재배선 전 6판). 선수(700냥)와 엽전 눈(170냥)이 뿌리가 되면서 초반 선택지가 둘
늘었다 — 가격이 그대로라 순서만 자유로워진 것.


## 2026-08-31 — N11-15 연마 확장 + 단계적 개방

노드 69 → 79 (연마 17 → 27). 우치 시점 총액 117,620 → 141,340냥, 만렙 113 → 135판.
1판 12딩·최대 가뭄 5판은 그대로 — 새 노드가 전부 기존 사슬의 아래쪽에 붙어 초반
구매 흐름을 건드리지 않기 때문이다.

단계적 개방으로 첫 화면 노드 수가 크게 줄었다(우치 29 → 6, 공용 35 → 5+개방분).
"살 수 있는 것"이 화면에 몇 개만 보이므로 선택 피로가 줄고, 구매가 지도를 여는
행위가 된다. 캡은 노드 추가에 맞춰 재산출했다(죽은 랭크 0).

## 2026-08-31 — N11-20d 함정 무기 두 종 (배치 측정)

오너: "빌드가 조잡하지 않은지, 출시할 정도 퀄리티". `--batch --seed=7`로 우치의 직접
획득 무기 7종을 단독 런으로 재보니 dps가 23.8~172.5, **7.2배** 벌어져 있었다. 격차의
바닥이 결계와 신장 소환 — 그 계열을 미는 빌드가 곧 함정이 된다.

| 무기 | 전 | 후 | 바꾼 것 |
|---|---:|---:|---|
| 결계 | 23.8 | **46.5** | 피해 2.6→5.2, 반경 45→74px, 틱 0.6→0.42초, 랭크당 성장 상향 |
| 신장 소환 | 38.9 | **63.5** | 피해 12→16, 소환수 공격 대기 0.65→0.5초, 지속 11→12초 |
| (대조) 혼불 | 79.1 | 91.6 | 미변경 — 시드 편차 |

격차 7.2배 → **3.6배** (46.5~168.3). 결계는 여전히 최저지만 장판은 통제형이라 dps가
낮은 것이 정상 범위다. 한 축만 만졌다 — 다른 무기의 수치는 건드리지 않았다.

**미해결(구조)**: 계열마다 직접 획득 가능한 무기가 1종뿐이고(봉인만 4종), 진화 22종은
전부 개조 결과물이라 승격이 불가능하다. 슬롯이 4칸인데 기룡은 기본 4종·초희는 3종이라
"고를 것"이 없다 — 빌드 선택이 성립하려면 계열당 기본 무기가 2~3종 필요하다. 별도 계획.
